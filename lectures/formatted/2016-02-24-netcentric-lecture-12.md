...
![f-4-27]({{ site.baseurl }}/img/lectures/f-4-27.png)
...
_static:_ routes change slowly over time<br>
...
$$C(x,y)$$: link cost from $$x$$ to $$y$$ = $$\infty$$ if no direct neighbors.<br>
$$D(v)$$: current value of cost of path from source to destination $$v$$<br>
$$P(v)$$: predecessor in route along the path from source to destination<br>
$$N'$$: set of nodes whose least cost is known<br>
...
_step 1:_<br>
$$D(v)=min(D(v), D(w)+C(w,v))$$ $$D(w)=min(D(w), D(x)+C(x,w))$$ $$=5, (1+3)$$ $$D(y)=min(D(y), D(x)+C(x,y))$$ $$=\infty, (1+1)$$<br>
_step 2:_<br>
$$D(w)=min\{D(w), D(y)+C(y,w)\}$$ $$=4,(2+1)$$ $$D(z)=\{D(z), C(y,z)+D(y)\}$$ $$=\infty, (2+2)$$<br>
_step 3:_<br>
$$D(w)=min\{D(w), D(v)+C(v,w)\}$$ $$=3,(2+3)$$<br>
_step 4:_<br>
$$D(z)min\{D(z), D(w)+C(w,z)\}$$ $$=4,(3+5)$$<br>
